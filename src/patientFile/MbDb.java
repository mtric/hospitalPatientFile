 ...
 * MysqlConnectionPoolDataSource, der dazu fuehrt, dass kein Connect zu MariaDB
 * aufgebaut werden kann (au�er der User hat kein Passwort) . Vgl. hier: 
 * https://mariadb.atlassian.net/browse/MDEV-5155
 * Ohne PW klappts. Grosser Mist!
 * Workaround: Aeltere Version des MySQL-Connectors einsetzen, z. B. 
...
/**
 * Ueber diese Bean erfolgt der Connect zu Datenbank. Die Verwaltung der Ver-
 ...
 * faces-config.xml gelesen. Alternativ ist auch ein JNDI lookup moeglich.
 * 
 * Wichtig: Dem Projekt muss der verwendete Datenbanktreiber hinzugefuegt werden,
...
public class MbDb  implements Serializable {
  ...
  public MbDb() { 
    log( "Creating patientFile.MbDb at " + new Date() );
  }
  ...
  /**
   * Logfile setzen (managed property) und Logwriter oeffnen
  ...
  public void setLogfile( String s ){ 
    ...
    try {     
      if( pwLog == null ) {
        ...
        if( ! fn.exists() ) {
          ...
          if( fParent != null && (! fParent.exists()) ) {
          	System.out.println( "Pfad " + fParent.getAbsolutePath() + " fuer Logfile wird angelegt..." );
          	fParent.mkdirs();
          }
        }
                
        pwLog = new PrintWriter( new FileOutputStream( fn ));              
        // Zusaetzliche Infos in Log-File ausgeben. Hilft beim debuggen:        
      ...
      err.println( "Fehler: Kann Verzeichnis fuer " + logfile 
                                                   + " nicht anlegen!" );
      ex.printStackTrace();
    }
      ...
      err.println( "Fehler: Kann file " + logfile + " nicht Oeffnen!");
      ex.printStackTrace();
    }    
  }
  ...
  /**
   * Verbindung zur Datenbank an den Pool zurueck geben
  ...
  /**
   * Stellt eine Connection zur Verfuegung. 
  ...
  /** 
   * Gibt einige Infos zur aktuellen connection ueber das log file aus.
   * @param con
   * @throws SQLException
   */
